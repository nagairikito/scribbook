...
class BlogService extends Service
{
    ...
    public const CLOUDINARY_URL = 'https://res.cloudinary.com/dqyo04d6k/image/upload/v1754640091/';
        ...
        try {
            ...
            if($inputData['thumbnail_name'] != 'noImage.png') {
                if(app()->environment('production')) {
                    ...
                    $renameImage = $this->updateBase64Image([$inputData['thumbnail_img']], [$inputData['blog_unique_id'] . '_' . $inputImageName], $targetBlog['blog_unique_id'], 'blog_thumbnail_images');
                    $inputData['thumbnail_name'] = $this::CLOUDINARY_URL . 'blog_thumbnail_images/'. $renameImage;
                } else {
    ...
    {
        if(count($imageBase64Texts) > 0) {
if($folderPath === 'blog_contents_images') {
    dd($imageBase64Texts);
}
            for($i=0; $i<count($imageBase64Texts); $i++) {
                if(preg_match('/^data:image\/[a-zA-Z]+;base64,/', $imageBase64Texts[$i]) === 1) {
                // 新規画像保存時
                    // 例: data:image/png;base64,iVBORw0KGgoAAAANS...
                    $base64Image = $imageBase64Texts[$i];
                    ...
                    if(app()->environment('production')) {
                        ...
                        $uploaded = $cloudinary->uploadApi()->upload(
                            ...
                            [
                                'folder'    => $folderPath,
                                'public_id' => pathinfo($imageNames[$i], PATHINFO_FILENAME),
                            ]
                        );
                        ...
                        // ファイル名と保存パス生成
                        $filePath = $folderPath . '/' . $imageNames[$i];
                ...
                // 既存画像保存時
                    ...
                    if(app()->environment('production')) {
                        ...
                        if($oldPublicId != null) {
                            ...
                            $registerImage = pathinfo($imageNames[$i], PATHINFO_FILENAME);
                        ...
                        if($targetImageFileName) {
                            foreach($targetImageFileName as $fileName) {
                                $changeNameFlag = Storage::disk('public')->move($fileName, $folderPath . $imageNames[$i]);
                                if($changeNameFlag == false) {
                                    throw new \Exception('画像名の更新に失敗しました');
                                }
                            }
        ...
        if($blogUniqueId != null && $blogUniqueId != '') {
            if(app()->environment('production')) {
                ...
                try {
                    ...
                    $expression = "public_id LIKE '*{$path}/{$blogUniqueId}*'";
                    // $expression = "folder:{$path} AND public_id LIKE '*{$blogUniqueId}*'";
                    // $expression = "resource_type:image AND type:upload AND prefix:{$path}/ AND public_id:*{$blogUniqueId}*";

                    $result = $cloudinary->searchApi()->expression($expression)->execute();
if($path === "blog_contents_images") {
dd($result);
}
